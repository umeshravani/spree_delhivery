      ...
      class FulfillmentsController < Spree::Admin::BaseController
        ...
        def create
          ...
          if params[:fulfillment_type] == 'manual'
            ...
            # --- THE SHOPIFY-GRADE MANUAL OVERRIDE ---
            # We bypass the strict ship! event and write directly to the DB
            # This handles both fresh shipments AND tracking number updates seamlessly.
            ActiveRecord::Base.transaction do
              ...
              # Ensure all inventory units are marked shipped
              @shipment.inventory_units.where.not(state: 'shipped').update_all(state: 'shipped')
  
              # Force order reconciliation
              @order.updater.update
            end
  
            # Dispatch the native Spree shipped email safely in the background
            Spree::ShipmentMailer.shipped_email(@shipment.id).deliver_later rescue nil
  
            carrier_name = @shipment.selected_shipping_rate&.name || "Manual Carrier"
